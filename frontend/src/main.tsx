...
function App() {
  ...
  const refreshProjects = async () => {
    const [data, settings] = await Promise.all([listProjects(), getProjectSettings()]);
    setProjects(data);
    if (data.length === 0) {
      const defaultProject = await createProject('Default Project');
      setProjects([defaultProject]);
      setProjectId(defaultProject.id);
      await saveLastProject(defaultProject.id);
      return;
    }

    const preferred = settings?.last_project_id as number | null;
    const exists = preferred && data.some((p: { id: number }) => p.id === preferred);
    if (exists) {
